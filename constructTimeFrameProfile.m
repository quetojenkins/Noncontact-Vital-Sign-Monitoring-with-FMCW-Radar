...
function[output_data] = constructTimeFrameProfile(input_data, channels, chirps, frames, samples)
    %Function Variables
    %----------------------------------------------------------------------
    ...
    output_data = complex(zeros(channels, frames, chirps, samples));
    
    %Reshape the unordered data into a 4-dimensional array
    input_data = reshape(input_data, samples, channels, chirps, []);
